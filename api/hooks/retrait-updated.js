...
// Database Webhook Supabase : UPDATE on public.retrait_orders
export default async function handler(req, res) {
  ...
  try {
    if (oldRecord && oldRecord.status !== record.status) {
      ...
      await sendTelegramAdmin(`🔄 Retrait #${record.id} → <b>${label}</b>`);
      if (record.whatsapp) {
        await sendWhatsApp(record.whatsapp, `Simba — Votre retrait #${record.id} (${record.montant} DJF) : ${label}`);
      }
    }
    return res.status(200).json({ ok: true });
  } catch (err) {
    console.error('[retrait-updated]', err);
